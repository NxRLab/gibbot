...
char str[100]; //used in case: TEST
...
void initialize_TIMER1(void){// for 5kHz interrupt frequency, PR1=2000-1. For 5 Hz, PR1=62500-1
	T1CONbits.TCS = 0;
...
void test() { //this function should go within the "switch" block once it is set up
    while (pwm!=desiredpwm) {
		write_duty(pwm);
		calc_effort(desiredpwm,pwm,kp,ki);
		sprintf(str, "Current duty cycle: %d Current proportional error: %d Current integral error: %d Current effort: %d\r\n",pwm,epro,eint,u);
