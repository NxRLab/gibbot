...
BOOL CreateAppWindow(const char* title, int width, int height, int bits, bool fullscreenflag)
{
    ...
    if (!InitGLSL())						        // Shader Init
    {
        CloseWindow();								// Reset The Display
        MessageBox(NULL,"Shader Initialization Failed.","ERROR",MB_OK|MB_ICONEXCLAMATION);
        return FALSE;								// Return FALSE
    }
...
"void main()"
"{"
"   vec2 tcoord = vec2(gl_TexCoord[0].st.x,gl_TexCoord[0].st.y);"
"   vec2 tcoordabove = vec2(gl_TexCoord[0].st.x,gl_TexCoord[0].st.y-blur);"
"   vec2 tcoordbelow = vec2(gl_TexCoord[0].st.x,gl_TexCoord[0].st.y+blur);"
"   vec2 tcoordright = vec2(gl_TexCoord[0].st.x+blur,gl_TexCoord[0].st.y);"
"   vec2 tcoordleft  = vec2(gl_TexCoord[0].st.x-blur,gl_TexCoord[0].st.y);"
"   gl_FragColor =  abs(texture2D(texRGB,tcoord)+texture2D(texRGB,tcoordbelow)+texture2D(texRGB,tcoordleft)+texture2D(texRGB,tcoordright)+texture2D(texRGB,tcoordabove))*gl_Color*0.2;"
"}\0";
...
bool InitGLSL()
{
    ...
    //== load vertex shader ==--
    unsigned char *shade = (unsigned char*) vertex_shader_source;
    mShaderVertex = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(mShaderVertex, 1, (const GLchar**)&shade, NULL);
    glCompileShader(mShaderVertex);
    if( !check_shader_compilation(mShaderVertex) )
        return false;

    //== load fragment shader ==--
    unsigned char *fragment = (unsigned char*) fragment_shader_source;
    mShaderFragment = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(mShaderFragment, 1, (const GLchar**)&fragment, NULL);
    glCompileShader(mShaderFragment);
    if( !check_shader_compilation(mShaderFragment) )
        return false;
...
void SetShaderUniform1f( const char *Variable, float Value )
{
    GLint myUniformLocation = glGetUniformLocation(mShaderProgram, Variable);
    glUniform1f(myUniformLocation,Value);
}

int DrawGLScene(Surface *surf, int width, int height, float BlurIntensity)
{
    if(surf==NULL)
        return true;
    ...
    SetShaderUniform1f("blur",BlurIntensity/512.0f);
