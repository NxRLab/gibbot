...
public class Gibbot {
	...
	public void draw(Graphics g){ //Handles its own redrawing so the panel class doesn't have to deal with all the variables
		...
		/*arm.setRoundRect(pivotX-radius, pivotY-radius, ARMW, ARMH, radius, radius);
		leg.setRoundRect(pivotX-radius, pivotY-radius, LEGW, LEGH, radius, radius);
		
		//transform.translate(5,5);
		transform.rotate(calcRotAngle(armX, armY),pivotX, pivotY);
		rotatedArm=transform.createTransformedShape(arm);
		transform.rotate(calcRotAngle(legX, legY),pivotX, pivotY);
		rotatedLeg=transform.createTransformedShape(leg);
