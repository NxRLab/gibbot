...
namespace Core
{
    ...
    public:
        ...
        void            SetAxisAngle( const cVector3<T> &aa )
        {
            ...
            cVector3<T> v = aa.Normalized() * sin(radians/2.);
        ...
        inline cVector3<T> AxisAngle() const
        {
            ...
            T radians = ((T)2.0)*acos(mW*s);

            cVector3<T> v(mX*s, mY*s, mZ*s);
        ...
        /// <summary>Initialize quaternion to a rotation along axis.</summary>
        template<typename U, typename V> 
        inline void SetAngleAxis( const Core::cVector3<U> &pvAxis, V angle )
        {
            T f = angle / 2;
            T s = sin(f);
            mW = cos(f);
            Core::cVector3<U> v( pvAxis.Normalized() );
            mX = s * v.X();
            mY = s * v.Y();
            mZ = s * v.Z();
