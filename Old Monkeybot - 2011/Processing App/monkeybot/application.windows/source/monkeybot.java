...
public class monkeybot extends PApplet {
 ...
 //   text(gotLabel,15,745); // what has been sent
  ...
  if (!SerialOK) {
    ...
    if(!StartOK) {
      ...
      stroke(0,100);
      pushMatrix();
      segment(290, 350, PApplet.parseFloat(angle1)*PI/180.0f); 
      segment(segLength, 0, PApplet.parseFloat(angle2-angle1)*PI/180.0f);
      popMatrix();
      noStroke();
      strokeWeight(1.0f);
...
class Button
{
  int x, y;
  int w, h;
  int basecolor, highlightcolor, clickcolor, activecolor, inactivecolor;
  PFont thefont;
  boolean over = false;
...
public void segment(float x, float y, float a) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "monkeybot" });
  }
}
